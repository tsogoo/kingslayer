...
class Robot:
    ...
    def move_handle(self, moves, turn):
        print("move_handling")
        self.config = get_config(self.config_data, "robot")
    ...
    def move_z_gode_array(self, down=True):
        step = 8
        delay = 300
        arr = []
        if down:
            start_angle = get_config(self.config, "z_lift:up_z")
            end_angle = get_config(self.config, "z_lift:down_z")
        else:
            start_angle = get_config(self.config, "z_lift:down_z")
            end_angle = get_config(self.config, "z_lift:up_z")

        step_angle = (end_angle - start_angle) / step
        return ["SET_SERVO servo=servo_up angle={}".format(end_angle), "G4 P{}".format(delay)]
        for i in range(step):
            start_angle = start_angle + step_angle
            arr.append("SET_SERVO servo=servo_up angle={}".format(start_angle))
            if step > 1:
                arr.append("G4 P{}".format(delay))
        return arr
